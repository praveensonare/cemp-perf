...
function Solarnode({ data }) {
  const [wsData, setWsData] = useState(null);
  const [isHovered, setIsHovered] = useState(false);
  const [position, setPosition] = useState({ top: 400, left: 590 });
  ...
  useEffect(() => {
    ...
    if (
      ...
      messages?.live_schematic?.solar?.direction !== 0
    ) {
      setShowImage(true);
    } else {
      setShowImage(false);
  ...
  //   if (
  //     messages?.project_id === projectSiteName &&
  //     messages?.live_schematic?.solar.direction !== null &&
  //     messages?.live_schematic?.solar.direction !== undefined
  //   ) {
  ...
  const rotationStyle =
    ...
    filterdata?.live_schematic?.solar?.direction === 1
  ...
  return (
    <>
      ...
      filterdata &&
      filterdata?.project_id === projectSiteName &&
      filterdata.live_schematic.solar.direction !== 0 ? (
        <img
          src={data.src}
          alt={data.label}
      ...
      {/* {showImageCard && messages && messages?.project_id === projectSiteName
        ? ReactDOM.createPortal(
            <div
              style={{
                top: position.top, // adjust this value as needed
                left: position.left, // adjust this value as needed
              }}
              className="nodes-popup"
            >
              <h6 style={{ fontWeight: "bold" }}>Solar</h6>
              {messages?.live_schematic?.solar ? (
                <div>
                {Object.entries(messages.live_schematic.solar)
